# R9 完成定义逐项审计（2026-08-20 快照）

> 审计日期：2026-08-20
> Godot：4.8 dev `4173760fdf6c2c722e82e08cb58e55f34c9efd80`
> 证据范围：提交 `06ccc87` 的 R9 基线；下列截图、数量和自动诊断哈希是该提交的历史快照。
...
`capture_r9_mature_presentation.gd` 在该快照生成 12 张 `640×360 RGBA` PNG。启用实际动画后的 SHA-256：
...
该快照的功能回归集数量为 G6 11、G4 8、R7 10、UI 6；连同 R9 共 47 张。所有文件均经 `file` 确认为
`640×360 8-bit RGBA`。
...
## 5. 2026-08-21 后续变更与重建要求

IES 已把 UI 基线从 6 张扩充为 9 张，因此当前完整截图集合应为 G6 11、G4 8、R7 10、UI 9、
R9 12，共 50 张；上文 47 张和 12 个 R9 SHA-256 不能证明 IES 之后的构建。随后 Content CLI
contract v2、Definition 可用性与显式 StoryBinding 迁移继续改变运行时和内容校验边界。

这些后续变更稳定后，必须重新执行工程、内容、地图、全量测试、UI/R9 截图和四段自动动态诊断，
再新建一份带 build commit 的审计记录；不要覆写本节的历史哈希，也不要在重建完成前把自动门或
静态门记为当前已证明。R9 缺少真人录像与五人盲测的结论不受此次重建影响。
...
R9 完成定义由四类证据合取：自动门、十二张静态图、四段真人动态录像、五人首次玩家盲测。
2026-08-20 快照证明了前两类，但后续变更需要重新建立当前自动/静态证据；真人动态和盲测仍缺失，
因此总体状态必须保持“进行中”。下一次完成状态变化只能来自按
