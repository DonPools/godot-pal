...
UI 基线必须生成九张 `640 x 360` RGBA PNG：
...
- `07_inventory_categories.png`：行囊分类、容量、物品图标、数量、说明与使用/快捷/丢弃动作同时可读。
- `08_equipment_comparison.png`：法器槽、候选法器、说明和 HP/真气/攻击差值在同页成立。
- `09_skill_loadout.png`：已学技能列表、数据图标和三个可配置战斗槽完整可读。
...
- 菜单五个页签在 `640 x 360` 内不重叠；行囊分类与详情、法器比较、三个技能槽、确认丢弃和系统
  入口都能使用键盘/鼠标/手柄完成，隐藏页不保留焦点。
