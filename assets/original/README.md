...
`ui/cursors/` 保存以 SVG 路径直接编写的移动、攻击、互动和不可达光标；`ui/actions/` 保存普攻、
三个技能、丹药与闪避动作图标；`ui/items/` 保存返青草、两件法器、食炁岩心和阵芯剑符图标。
这些 SVG 使用统一的暗边、暗金、青蓝、草木绿与朱红色板，源文件本身就是可编辑运行素材。
它们由文本路径和基础几何确定性维护，不经过不确定的外部生成步骤；ItemDefinition/SkillDefinition
直接引用对应 Texture2D，使菜单与 HUD 共用同一内容图标真相。
