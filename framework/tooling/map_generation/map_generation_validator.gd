...
func validate_profile(profile: MapGenerationProfile, map_scene: Node = null) -> Array[Dictionary]:
	...
	if profile.schema_version != 2:
		diagnostics.append(_diagnostic(
			...
			"3D map generation requires schema 2, got %d" % profile.schema_version,
			profile_path,
			"schema_version"
		))
	...
	if profile.cell_size_3d.x <= 0.0 or profile.cell_size_3d.y <= 0.0:
		diagnostics.append(_diagnostic(
	...
	if (
		profile.target_scene_path.is_empty()
		or not ResourceLoader.exists(profile.target_scene_path, "PackedScene")
	):
		diagnostics.append(_diagnostic(
...
func validate_plan(profile: MapGenerationProfile, plan: MapGenerationPlan) -> Array[Dictionary]:
	...
	if plan.terrain_tags.size() != expected_cells:
		diagnostics.append(_diagnostic(
			"map_generation_ground_incomplete",
			"plan contains %d ground cells, expected %d"
			% [plan.terrain_tags.size(), expected_cells],
	...
	if connected_anchors.size() > 1:
		var reachable := _walkable_reachable(
			plan,
			plan.resolved_anchor_cells[connected_anchors[0].id]
		)
...
func validate_baked_scene(profile: MapGenerationProfile, map_scene: Node) -> Array[Dictionary]:
	...
	if map_scene == null:
		return diagnostics
	_validate_baked_scene(profile, map_scene, diagnostics)
...
func _validate_biome(
	...
	if biome.id.is_empty():
		diagnostics.append(_diagnostic(
			"map_generation_biome_id_missing",
			"biome ID is empty",
			profile_path,
			"biome.id"
		))
	...
	for index: int in biome.prop_rules.size():
		...
		if not instance is Node3D:
			diagnostics.append(_diagnostic(
				...
				"3D prop scene root must be Node3D: %s" % rule.id,
				profile_path,
				"biome.prop_rules",
				String(rule.id)
			))
		...
		instance.free()
	_validate_modules(profile, diagnostics)
...
func _validate_anchors(
	...
	for index: int in profile.anchors.size():
		...
		if anchor.use_scene_node:
			if map_scene == null:
				continue
			var anchor_node := map_scene.get_node_or_null(anchor.node_path) as Node3D
			if anchor_node == null:
				diagnostics.append(_diagnostic(
					"map_generation_anchor_node_missing",
					"3D anchor node does not exist: %s" % anchor.node_path,
					profile.authoring_source_path(),
					"anchors",
					String(anchor.id)
				))
			...
			if world_root != null:
				cell = profile.world_to_cell(
					_transform_relative_to_ancestor(anchor_node, world_root).origin
				)
...
func _validate_target_scene(
	...
	for path: NodePath in [^"WorldRoot", ^"WorldRoot/Terrain", ^"WorldRoot/SpawnPoints"]:
...
func _contains_forbidden_gameplay_node(node: Node) -> bool:
	if node is StoryInteractable3D or node is EncounterSource3D:
		return true
	for child: Node in node.get_children():
...
func _validate_modules(
	...
	if biome.road_overlay_module != null:
		var road_modules: Array[MapGenerationModule3D] = [biome.road_overlay_module]
