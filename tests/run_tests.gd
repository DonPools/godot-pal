...
func _test_scene_smoke() -> void:
	...
	var hall := game_root.scene_stack.current_scene() as LabMapGameScene
	_expect(hall != null and hall.map_id == &"map.lab.inn_hall", "new game should load the hall")
	if hall != null:
		_expect(hall.ground_layer.get_used_cells().size() == 150, "hall should build its TileMapLayer")
	...
	await _drain_dialogue(game_root)
	var courtyard := game_root.scene_stack.current_scene() as LabMapGameScene
	_expect(
		courtyard != null and courtyard.map_id == &"map.lab.rain_courtyard",
		"travel should replace the active map"
	)
	if courtyard != null:
		_expect(courtyard.ground_layer.get_used_cells().size() == 150, "courtyard should build its TileMapLayer")
	...
	await _drain_dialogue(game_root)
	hall = game_root.scene_stack.current_scene() as LabMapGameScene
	if hall != null:
		hall.player.position = (hall.get_node("YSortRoot/Innkeeper") as Node2D).position
		hall._on_player_interact()
		await _drain_dialogue(game_root)
